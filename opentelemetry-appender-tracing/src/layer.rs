    ...
    L: Logger + Send + Sync,
{
    ...
    #[cfg(feature = "experimental_span_attributes")]
    span_name_attribute_key: Option<Key>,
}
    ...
    L: Logger + Send + Sync,
{
    ...
    /// event is emitted. The argument is the log attribute key where that
    /// tracing span name will be stored; it is **not** the span name itself.
    ///
    ...
    /// [`tracing::span!`]: https://docs.rs/tracing/latest/tracing/macro.span.html
    #[cfg(feature = "experimental_span_attributes")]
    pub fn with_tracing_span_name(mut self, span_name_attribute_key: impl Into<Key>) -> Self {
        self.span_name_attribute_key = Some(span_name_attribute_key.into());
        self
    }
    ...
    pub fn build(self) -> OpenTelemetryTracingBridge<P, L> {
        OpenTelemetryTracingBridge {
            ...
            #[cfg(feature = "experimental_span_attributes")]
            span_name_attribute_key: self.span_name_attribute_key,
        }
    }
}
    ...
    L: Logger + Send + Sync + 'static,
{
    fn on_event(
        ...
        #[cfg(feature = "experimental_span_attributes")]
        if self.span_attributes.is_some() || self.span_name_attribute_key.is_some() {
            ...
            if let Some(scope) = ctx.event_scope(event) {
                ...
                for span_ref in scope.from_root() {
                    if self.span_name_attribute_key.is_some() {
                ...
                if let (Some(key), Some(span_name)) =
                    (self.span_name_attribute_key.as_ref(), current_span_name)
                {
                    log_record.add_attribute(key.clone(), AnyValue::from(span_name));
                }
            }
        }
